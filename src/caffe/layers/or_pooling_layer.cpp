      ...
      for (int c = 0; c < channels_; ++c) {
        for (int ph = 0; ph < pooled_height_; ++ph) {
          for (int pw = 0; pw < pooled_width_; ++pw) {
            ...
            for (int g = 0; g < kernel_w_; g++) {
              bottom_data = bottom[0]->cpu_data() + bottom[0]->offset(n, c * kernel_w_ + g);
              if (bottom_data[pool_index] > top_data[pool_index]) {
                top_data[pool_index] = bottom_data[pool_index];
                if (use_top_mask) {
                  top_mask[pool_index] = static_cast<Dtype>(g);
                } else {
                  mask[pool_index] = g;
                }
              }
      ...
      for (int c = 0; c < channels_; ++c) {
        for (int ph = 0; ph < pooled_height_; ++ph) {
          for (int pw = 0; pw < pooled_width_; ++pw) {
            ...
            bottom_diff[bottom[0]->offset(n, c * kernel_w_ + bottom_index)] += top_diff[index];
          }
        }
